...
static void normalize_poly(struct distmap *d, struct poly3 *poly_x, struct poly3 *poly_y)
{
    const float ox = d->p.poly_corner[0];
    const float oy = d->p.poly_corner[1];
    const float ux = d->p.poly_width[0];
    const float uy = d->p.poly_width[1];
    const float vx = d->p.poly_height[0];
    const float vy = d->p.poly_height[1];
    const float s = 1.f / (ux * vy - uy * vx);
    poly_x->a = s * (vy*poly_x->a - vx*poly_y->a);
    poly_y->a = s * (ux*poly_y->a - uy*poly_x->a);
    poly_x->b = s * (vy*poly_x->b - vx*poly_y->b);
    poly_y->b = s * (ux*poly_y->b - uy*poly_x->b);
    poly_x->c = s * (vy*poly_x->c - vx*poly_y->c);
    poly_y->c = s * (ux*poly_y->c - uy*poly_x->c);
    poly_x->d = s * (vx*(oy - poly_y->d) + vy*(poly_x->d - ox));
    poly_y->d = s * (ux*(poly_y->d - oy) + uy*(ox - poly_x->d));
}
...
int ngli_distmap_add_poly3(struct distmap *d, int shape_id, const float *x, const float *y)
{
    ...
    struct poly3 poly_x = {x[0], x[1], x[2], x[3]};
    struct poly3 poly_y = {y[0], y[1], y[2], y[3]};

    normalize_poly(d, &poly_x, &poly_y);
...
void ngli_distmap_get_shape_coords(const struct distmap *d, int shape_id, float *dst)
{
    ...
    const float gw = d->p.shape_w * scale_w;
    const float gh = d->p.shape_h * scale_h;
