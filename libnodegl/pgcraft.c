                          ...
                          const struct pgcraft_uniform *uniform, int stage)
{
    ...
    if (!ngli_darray_push(&s->pipeline_desc.uniforms, &pl_uniform_desc))
        return NGL_ERROR_MEMORY;
    if (!ngli_darray_push(&s->pipeline_data.uniforms, &uniform->data))
        return NGL_ERROR_MEMORY;
    return 0;
}
...
static int inject_texture_info(struct pgcraft *s, struct pgcraft_texture_info *info, int stage)
{
    for (int i = 0; i < NGLI_INFO_FIELD_NB; i++) {
        ...
        if (is_sampler_or_image(field->type)) {
            ...
            if (!ngli_darray_push(&s->pipeline_desc.textures, &pl_texture_desc))
                return NGL_ERROR_MEMORY;
            if (!ngli_darray_push(&s->pipeline_data.textures, &info->texture))
                return NGL_ERROR_MEMORY;
        } else {
                        ...
                        const struct pgcraft_block *named_block, int stage)
{
    ...
    if (!ngli_darray_push(&s->pipeline_desc.buffers, &pl_buffer_desc))
        return NGL_ERROR_MEMORY;
    if (!ngli_darray_push(&s->pipeline_data.buffers, &named_block->buffer))
        return NGL_ERROR_MEMORY;
    return 0;
}
                            ...
                            const struct pgcraft_attribute *attribute, int stage)
{
    ...
    for (int i = 0; i < attribute_count; i++) {
        ...
        if (!ngli_darray_push(&s->pipeline_desc.attributes, &pl_attribute_desc))
            return NGL_ERROR_MEMORY;
        if (!ngli_darray_push(&s->pipeline_data.attributes, &attribute->buffer))
            return NGL_ERROR_MEMORY;
    }
...
static int filter_pipeline_elems(struct pgcraft *s, probe_func_type probe_func,
                                 ...
                                 struct darray *src_desc, struct darray* src_data, struct darray *dst_desc, struct darray* dst_data)
{
...
{
    const struct pipeline_uniform_desc *pipeline_uniform_descs = ngli_darray_data(&s->filtered_pipeline_desc.uniforms);
    for (int i = 0; i < ngli_darray_count(&s->filtered_pipeline_desc.uniforms); i++) {
...
{
    const struct pipeline_texture_desc *pipeline_texture_descs = ngli_darray_data(&s->filtered_pipeline_desc.textures);
    for (int i = 0; i < ngli_darray_count(&s->filtered_pipeline_desc.textures); i++) {
...
static int probe_pipeline_elems(struct pgcraft *s)
{
    ...
    if ((ret = filter_pipeline_elems(s, probe_pipeline_uniform,   uniforms_info,   &s->pipeline_desc.uniforms,   &s->pipeline_data.uniforms,
                                     &s->filtered_pipeline_desc.uniforms, &s->filtered_pipeline_data.uniforms))   < 0 ||
        (ret = filter_pipeline_elems(s, probe_pipeline_buffer,    buffers_info,    &s->pipeline_desc.buffers,    &s->pipeline_data.buffers,
                                     &s->filtered_pipeline_desc.buffers,    &s->filtered_pipeline_data.buffers))    < 0 ||
        (ret = filter_pipeline_elems(s, probe_pipeline_texture,   uniforms_info,   &s->pipeline_desc.textures,   &s->pipeline_data.textures,
                                     &s->filtered_pipeline_desc.textures,    &s->filtered_pipeline_data.textures))   < 0 ||
        (ret = filter_pipeline_elems(s, probe_pipeline_attribute, attributes_info, &s->pipeline_desc.attributes, &s->pipeline_data.attributes,
                                     &s->filtered_pipeline_desc.attributes,    &s->filtered_pipeline_data.attributes)) < 0)
        return ret;
...
struct pgcraft *ngli_pgcraft_create(struct ngl_ctx *ctx)
{
    ...
    ngli_darray_init(&s->pipeline_desc.uniforms,   sizeof(struct pipeline_uniform_desc),   0);
    ngli_darray_init(&s->pipeline_desc.textures,   sizeof(struct pipeline_texture_desc),   0);
    ngli_darray_init(&s->pipeline_desc.buffers,    sizeof(struct pipeline_buffer_desc),    0);
    ngli_darray_init(&s->pipeline_desc.attributes, sizeof(struct pipeline_attribute_desc), 0);

    ngli_darray_init(&s->filtered_pipeline_desc.uniforms,   sizeof(struct pipeline_uniform_desc),   0);
    ngli_darray_init(&s->filtered_pipeline_desc.textures,   sizeof(struct pipeline_texture_desc),   0);
                       ...
                       const struct pgcraft_params *params)
{
    ...
    if (s->use_ublock) {
        ngli_assert(dst_desc_params->nb_uniforms == 0);
        for (int i = 0; i < NGLI_ARRAY_NB(s->ublock); i++) {
            struct block *block = &s->ublock[i];
            if (block->size) {
                struct buffer *buffer = s->ubuffer[i];
                dst_desc_params->ublock[i]  = block;
                dst_desc_params->ubuffer[i] = buffer; //TODO: remove
            }
        }
    } else {
        memset(dst_desc_params->ublock, 0, sizeof(dst_desc_params->ublock));
    }

    dst_desc_params->program            = s->program;
    dst_desc_params->uniforms_desc      = ngli_darray_data(&s->filtered_pipeline_desc.uniforms);
    dst_desc_params->nb_uniforms        = ngli_darray_count(&s->filtered_pipeline_desc.uniforms);
    dst_desc_params->textures_desc      = ngli_darray_data(&s->filtered_pipeline_desc.textures);
    dst_desc_params->nb_textures        = ngli_darray_count(&s->filtered_pipeline_desc.textures);
    dst_desc_params->attributes_desc    = ngli_darray_data(&s->filtered_pipeline_desc.attributes);
    dst_desc_params->nb_attributes      = ngli_darray_count(&s->filtered_pipeline_desc.attributes);
    dst_desc_params->buffers_desc       = ngli_darray_data(&s->filtered_pipeline_desc.buffers);
    dst_desc_params->nb_buffers         = ngli_darray_count(&s->filtered_pipeline_desc.buffers);


    dst_data_params->uniforms           = ngli_darray_data(&s->filtered_pipeline_data.uniforms);
    dst_data_params->nb_uniforms        = ngli_darray_count(&s->filtered_pipeline_data.uniforms);
    dst_data_params->textures           = ngli_darray_data(&s->filtered_pipeline_data.textures);
    dst_data_params->nb_textures        = ngli_darray_count(&s->filtered_pipeline_data.textures);
    dst_data_params->attributes         = ngli_darray_data(&s->filtered_pipeline_data.attributes);
    dst_data_params->nb_attributes      = ngli_darray_count(&s->filtered_pipeline_data.attributes);
    dst_data_params->buffers            = ngli_darray_data(&s->filtered_pipeline_data.buffers);
    dst_data_params->nb_buffers         = ngli_darray_count(&s->filtered_pipeline_data.buffers);
...
void ngli_pgcraft_freep(struct pgcraft **sp)
{
    ...
    ngli_darray_reset(&s->pipeline_desc.uniforms);
    ngli_darray_reset(&s->pipeline_desc.textures);
    ngli_darray_reset(&s->pipeline_desc.buffers);
    ngli_darray_reset(&s->pipeline_desc.attributes);

    ngli_darray_reset(&s->filtered_pipeline_desc.uniforms);
    ngli_darray_reset(&s->filtered_pipeline_desc.textures);
    ngli_darray_reset(&s->filtered_pipeline_desc.buffers);
    ngli_darray_reset(&s->filtered_pipeline_desc.attributes);
